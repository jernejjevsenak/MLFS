...
V_general_halfPeriod = function(df, data_volF_param = data_volF_param){
  ...
  df_PCAB <- mutate(df_PCAB,
               D = sqrt(4*BA_mid/pi) * 100/10, #cm -> dm

               H = height_mid * 10, # m -> dm

               volume_mid = (pi/4)*(a*D^2*H+b*D^2*H*log(D)^2+c*D^2+d*D*H+e*H+f*D) / 1000)   # dm3 -> m3
  ...
  df_ABAL <- mutate(df_ABAL,
                    ...
                    volume_mid = (pi/4)*(a*D^2*H+b*D^2*H*log(D)^2+c*D^2+d*D*H+e*H+f*D+g)/1000)   # dm3 -> m3
  ...
  df_PISY <- mutate(df_PISY,
                    ...
                    volume_mid = (pi/4)*(a*D^2*H+b*D^2*H*log(D)^2+c*D^2+d*H)/1000)   # dm3 -> m3
  ...
  df_QUSP <- mutate(df_QUSP,
                    ...
                    volume_mid = (pi/4)*(a*D^2*H+b*D^2+c*D*H+d*H+e*D+f) / 1000)   # dm3 -> m3
